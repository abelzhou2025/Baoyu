...
function toReadableParagraphs(segments, { keepBrackets, sentencesPerParagraph = 4 } = {}) {
  const cleaned = cleanSegments(segments, { keepBrackets });

  // Join all text first to properly handle sentence boundaries
  const fullText = cleaned.join(" ");

  // Split by sentence endings (. ! ? 。 ！？) but keep the delimiter
  // This pattern matches sentence endings while preserving them
  ...
  while ((match = sentencePattern.exec(fullText)) !== null) {
    // Extract text from last position to after the sentence ending
    const sentenceEnd = match.index + match[0].length;
    const sentence = fullText.slice(lastIndex, sentenceEnd).trim();
    if (sentence) {
  ...
  // Add any remaining text that doesn't end with sentence punctuation
  ...
  // If no sentences found, fall back to original segments
  ...
  for (const sentence of sentences) {
    ...
    // Start a new paragraph after accumulating enough sentences
...
async function getVideoTitle(url) {
  // Try YouTube oEmbed API first (no API key needed, works without yt-dlp)
...
function sanitizeFilename(title) {
  ...
  return title
    ...
    .replace(/^\.+$/, '')          // Remove dots that would make hidden files
    .slice(0, 200);                // Limit length
}

async function cmdTranscript({ url, lang, timestamps, segments, readable, keepBrackets, extra }) {
  ...
  if (isYouTubeUrl(url)) {
    ...
    if (id) {
      try {
        ...
        if (segments) {
          // Output as individual segments (one per line)
          const cleaned = cleanSegments(transcript.map((e) => decodeHtmlEntities(e.text)), { keepBrackets });
          for (const line of cleaned) {
            process.stdout.write(line + "\n\n");
          }
          return;
        }
        // Default to readable format (article-style paragraphs)
        const paragraphs = toReadableParagraphs(transcript.map((e) => decodeHtmlEntities(e.text)), { keepBrackets });
  ...
  try {
    ...
    const segmentsData = subtitlePath.endsWith(".srt") ? parseSrt(raw) : parseVtt(raw);
    ...
    if (timestamps) {
      ...
      const paragraph = toParagraph(segmentsData, { keepBrackets });
      process.stdout.write(paragraph + "\n");
      return;
    }
    if (segments) {
      // Output as individual segments (one per line)
      const cleaned = cleanSegments(segmentsData, { keepBrackets });
      for (const line of cleaned) {
        process.stdout.write(line + "\n\n");
      }
      return;
    ...
    // Default to readable format (article-style paragraphs)
    const paragraphs = toReadableParagraphs(segmentsData, { keepBrackets });
    for (const para of paragraphs) {
      process.stdout.write(para + "\n\n");
    }
  } finally {
...
function usage() {
  ...
  return [
    "usage:",
    `  ${rel} transcript --url 'https://…' [--lang en] [--timestamps] [--segments] [--keep-brackets] [-- <yt-dlp extra…>]`,
    ...
    "",
    "transcript modes:",
    "  default      : article-style paragraphs with video title (recommended)",
...
async function main() {
  ...
  const segments = Boolean(opts.segments);
  const readable = Boolean(opts.readable);
  const keepBrackets = Boolean(opts["keep-brackets"]);
  const extra = opts.extra || [];

  if (cmd === "transcript") {
    await cmdTranscript({ url, lang, timestamps, segments, readable, keepBrackets, extra });
    return;
  }
  if (cmd === "download") {
